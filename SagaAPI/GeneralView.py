...
class GeneralView(Resource):
    ...
    def get(self, command=None):
        ...
        if command=='UpdatedInstallation':
            ...
            if os.path.exists(safe_join(self.rootpath,'static/executable/Saga.exe')):
                result = send_from_directory(safe_join(self.rootpath,'static/executable/'),'Saga.exe')
                ...
                resp.data = json.dumps({ "response": safe_join(self.rootpath,'static/executable/Saga.exe') + " missing"  })
                return resp
        resp = make_response()
        resp.headers['status'] = 'Failed'
